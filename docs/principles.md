...
This is a record of how this web app is structured.
...
- The system is fully startable and stoppable from any process (like a test, a
...
that needs the database does not reach into global state, it accepts a `db`
...
Avoid them. Occasionally there are a small number of things that genuinely
*are* environmental: the port a platform requires binding to, secrets
injected by a secret store, anything that legitimately differs per
machine.
...
env/dev/resources/dev.edn          ; dev profile
env/test/resources/test.edn        ; test profile
env/prod/resources/prod.edn        ; prod profile (baked into the uberjar)
```

`reader.main` exposes the loading pipeline as plain functions —
`load-config`, `merge-profiles`, `prep-config`, `exec-config` — and
holds the always-loaded list as `core-profiles`. An additional profile
is named on the command line (`clojure -M:dev dev.edn`) or composed
in the dev `dev.clj` namespace. Meta-merge layers later profiles over
earlier ones; the merged map is what `ig/init` sees.

For the small number of values that *must* come from the environment
(PORT, secrets), use EDN reader literals (`#env`, `#env/opt`,
`#env/long`, `#env/bool`, `#env/secret`, `#resource`) registered with
the Integrant reader.

Configuration stays *in one place* (the .edn files) even when some of its
values come from outside. Aliases and/or explicit arguments load the
relevant profiles (see below).

## 3. Use aliases for profile selection. Not env vars.

A "profile" (dev, test, prod) is a property of how a process was started,
not of the runtime environment. Use `deps.edn` aliases that both put the
right `env/<env>/resources/` on the classpath and pass the profile filename
to `reader.main`:

```
clojure -M:dev          # -m reader.main dev.edn
clojure -M:test         # -m kaocha.runner   (test.edn is on the classpath)
clojure -M:prod         # -m reader.main prod.edn  (used by the uberjar)
```

The alias loads the relevant profile. This is composable with other
aliases (`:dev:test` works), trivial to reason about, and removes and
entire class of "what's the value of `WHATVER_ENV_VAR` again?" bugs.
...
somewhere; buttons do something. Doing this properly gets us
accessibility, SEO, and screen-reader behavior mostly for free.
...
`core/`, `shell/`, `domain/`, `infrastructure/`, etc. Do not.
That's an organizing principle for thinking (keeping pure logic
separate from an impure shell), but it should not be reflected in the
directory layout. In Clojure, idiomatic organization is by domain:
...
imperative shell that calls them. The organizing principle persists button
the file tree remains conventional.

## 7. Tasks belong in `bb.edn`. Never in a Makefile.

Makefiles are a foreign syntax, full of tab-vs-space traps, and they
encode build logic in a language that this project does not use.
**All developer tasks**, e.g. running the app, running tests, linting,
formatting, building the uberjar, building the container image,
deploying, starting local infra, etc., go in `bb.edn` as Babashka tasks.
...
beyond what trivially fits in an inline `(shell ...)` form.
...
when we need it, and only extend it when there enough concrete use cases
to design the abstraction. Three concrete instances is better than one
premature abstraction.

This applies to data, too: don't add database columns "in case we need
them." Don't precompute hashes we're not using.
...
article has a canonical URL and an extracted reading-time estimate. They
all have different attributes. Putting them into a shared schema forces
nullable columns and pushes type-discrimination into application
...
relationships also get their own tables, e.g. affiliations are
first-class because they have multiple authors, multiple publications,
a type, a URL, and they outlive any individual article. They are not
just a column on `articles`. We can denormalize the schema in the future
if access patterns necessitate it.
...
mess against a denormalized one.
...
When starting a new feature, the first goal is to get an end-to-end
implementation running first with trivial/placeholder functionality
before implementing the "real" version, e.g. a hello-world page served
by the real HTTP server, in the real container, on the real platform,
behind the real DNS, and a real CI pipeline that deploys it.
...
Building each layer fully before integrating is how projects end up
with a sophisticated domain layer that has never run on production
infrastructure that is a PITA to actually deploy.
...
3. Renders the result (Hiccup, JSON, redirect, etc)
...
that's awkward in pure HTML covers ~95% of the surface area of a
...
Anything that could matter if the process restarts mid-flight, e.g.
sending an email, extracting text from a freshly uploaded PDF, ingesting
an inbound newsletter, goes through a **durable job table** in Postgres.
A worker (in this case an Integrant-managed core.async loop) pulls
pending jobs, runs them, and updates state.

In-memory `chan`s are fine for things that are inherently transient,
like streaming progress updates to a logged-in user, coordinating
between parts of a single request. Anything user-visible after a restart
is durable.

## 17. Tests cover the system, not just the units.

Unit tests are cheap and worth writing for pure functions. But the
most valuable tests are the ones that stand up a real subset of the
Integrant system, hit it with real inputs, and assert on the outcome.
With Integrant + testcontainers, this is barely more code than a unit
test and much more confidence.
...
manual enough to need a wiki page, the wiki page is like a bug for which
...
locally, where the docs live (this folder), and the
license.
