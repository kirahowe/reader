...
(defn- el-text [^org.jsoup.nodes.Element parent css]
...
(defn fetch-metadata
  "Fallback metadata for `arxiv-id` from the arXiv Atom API, via `fetch-fn` (url ->
   body string or nil) — title/abstract/date and author *names* (no ORCID or
   institutions; OpenAlex is the source for those). The graph shape matches
   reader.papers.openalex/normalize so the job consumes it identically; the venue
   is arXiv (a preprint). nil when the paper isn't found."
  [fetch-fn arxiv-id]
  (when-let [xml (fetch-fn (str "https://export.arxiv.org/api/query?id_list=" arxiv-id))]
    (let [entry (.selectFirst (Jsoup/parse xml "" (Parser/xmlParser)) "entry")]
      (when-let [title (some-> entry (el-text "title"))]
        {:title     title
         ...
         :authors   (mapv (fn [n] {:name (str/trim (.text ^org.jsoup.nodes.Element n))})
